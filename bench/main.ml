...
module type Store = sig
  include
    Irmin_server.S
      with type Store.key = string list
       and type Store.contents = bytes
...
module Bootstrap_trace = struct
  ...
  let rec flatten_key_suffix = function
    | a :: b :: c :: d :: e :: f :: tl
      when is_2char_hex a && is_2char_hex b && is_2char_hex c && is_2char_hex d
           && is_2char_hex e && is_30char_hex f ->
  ...
  module Stats = struct
    ...
    module Summary = struct
      ...
      let summarise_op v : per_op =
        ...
        |> seq_mapi64 |> Seq.fold_left aux acc0
...
module Generate_trees_from_trace (Rpc : Store) = struct
  ...
  let exec_find t n i keys b in_ctx_id () =
    let { tree } = Hashtbl.find t.contexts (unscope in_ctx_id) in
    maybe_forget_ctx t in_ctx_id;
    Client.Tree.find tree keys >|= function
    | Ok None when not b -> ()
    | Ok (Some _) when b -> ()
  ...
  let add_commits repo max_ncommits commit_seq on_commit on_end stats check_hash
    ...
    let* tree = Client.Tree.empty repo >|= Error.unwrap "Tree.empty" in
    Hashtbl.add t.contexts 0L { tree };
...
let main_term =
  Term.(
    ...
    $ store_type $ freeze_commit $ flatten $ depth $ width $ nchain_trees
    $ nlarge_trees $ commit_data_file $ results_dir)
